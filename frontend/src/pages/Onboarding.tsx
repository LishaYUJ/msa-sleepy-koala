...
export const Onboarding: React.FC = () => {
  ...
  `}</style><section className="onboarding-card" aria-labelledby="onboarding-title"><div className="onboarding-icon"><Moon size={27} aria-hidden="true" /></div><h1 id="onboarding-title">Set your sleep goal for tonight.</h1><p className="onboarding-lede">Choose when you want to start getting ready for bed. You can change this anytime in Settings.</p><form onSubmit={finish}><div className="onboarding-field"><label htmlFor="cutoff-time"><Clock3 size={17} aria-hidden="true" /> Sleep goal time</label><input id="cutoff-time" type="time" min="21:00" max="23:59" value={cutoffTime} onChange={(event) => setCutoffTime(event.target.value)} required /></div><div className="onboarding-how"><CheckCircle2 size={22} aria-hidden="true" /><p className="onboarding-explainer">Each night, open the dashboard and select “I'm going to sleep” to record your bedtime check-in.</p></div>{error && <p className="onboarding-error" role="alert">{error}</p>}<button className="btn btn-primary" type="submit" disabled={!settings || isLoading}>{isLoading ? 'Saving...' : 'Finish setup and meet my koala'}</button></form></section></main>;
};
